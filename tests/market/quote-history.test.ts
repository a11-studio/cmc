...
describe("quote history", () => {
  ...
  it("returns the snapshot closest to 15 minutes ago", () => {
    rememberSnapshot(snapshot("2026-09-17T14:00:00.000Z", { BTC: 100 }));
    rememberSnapshot(snapshot("2026-09-17T14:15:00.000Z", { BTC: 110 }));
    ...
    expect(prior?.at).toBe("2026-09-17T14:15:00.000Z");
    expect(prior?.ageMinutes).toBe(15);
    expect(prior?.changePercent).toBeCloseTo((121 - 110) / 110 * 100, 6);
  });

  it("does not treat a two-minute-old quote as a 15-minute prior", () => {
    rememberQuotePoint({
  ...
  it("uses the observation time, not CoinMarketCap last_updated", () => {
    rememberSnapshot(snapshot("2026-09-17T14:30:00.000Z", { BTC: 99 }), "2026-09-17T14:15:00.000Z");
  ...
  it("compares current open interest to the 15-minute prior", () => {
    rememberQuotePoint({
  ...
  it("keeps one quote per minute so a burst of fetches cannot evict the 15m prior", () => {
    rememberQuotePoint({
      timestamp: "2026-09-17T14:15:01.000Z",
      prices: { BTC: 110 },
    });
    rememberQuotePoint({
      timestamp: "2026-09-17T14:15:59.000Z",
      prices: { BTC: 111 },
    });
