        ...
        <table className="w-full min-w-[880px] border-separate border-spacing-0 text-left text-sm">
          ...
          <tbody>
            {ranked.map((agent) => {
              ...
              return (
                // The row is the positioning context, so the link below stretches
                // across every cell and keeps native anchor behaviour.
                <tr
                  key={agent.id}
                  className="relative transition-colors hover:bg-white/[0.025]"
                >
                  <td className="border-t border-white/6 py-2.5 pr-4">
                    <Link
                      href={`/agents/${agent.id}`}
                      aria-label={`${agent.name} — ${agent.strategy}`}
